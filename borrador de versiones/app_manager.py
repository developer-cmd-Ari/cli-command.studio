...
from PySide6.QtWidgets import (
    ...
    QPushButton, QListWidget, QStackedWidget, QPlainTextEdit, QTabWidget,
...
class InstanceListItemWidget(QWidget):
    ...
    def __init__(self, instance_name, instance_id, manager_window):
        ...
        self.start_stop_button.setFocusPolicy(Qt.NoFocus) # Evita que el botón "robe" el foco
...
class InstanceManager(QMainWindow):
    ...
    def load_instances(self):
        ...
        for instance_id in instance_ids:
            instance_name = self.settings.value(instance_id)
            ...
            if os.path.exists(instance_dir):
                ...
                self.settings.remove(instance_id)
    ...
    def set_current_instance(self, instance_id):
        ...
        if instance_id in self.running_processes and self.running_processes[instance_id]:
            ...
            for index in sorted(self.console_widgets.get(instance_id, {}).keys()):
                console = self.console_widgets[instance_id][index]
                command = console.property("command")
                self.console_tabs.addTab(console, self.get_short_command_name(command))
        else:
    ...
    def close_console_tab(self, index):
        ...
        if not isinstance(widget, QPlainTextEdit): # Ignorar si no es una consola (ej. placeholder)
        ...
        if instance_id in self.running_processes and command_index in self.running_processes[instance_id]:
            ...
            process.kill()
            # La limpieza se hará en on_process_finished
    ...
    def start_instance(self):
        ...
        if not os.path.exists(cmd_file):
            QMessageBox.critical(self, "Error", "No se encontró el archivo 'comandos.txt'.")
            return
        ...
        if not commands:
            placeholder = QLabel("El archivo 'comandos.txt' está vacío o solo contiene comentarios.")
            placeholder.setAlignment(Qt.AlignCenter)
            self.console_tabs.addTab(placeholder, "Aviso")
            return
        ...
        for i, command in enumerate(commands):
            ...
            console_output.setProperty("instance_id", instance_id)
            console_output.setProperty("command_index", i)
            console_output.setProperty("command", command)
            
            self.console_widgets[instance_id][i] = console_output
            self.console_tabs.addTab(console_output, self.get_short_command_name(command))
            ...
            console_output.appendPlainText(f"--- Ejecutando: {command} ---\n")
    ...
    def handle_stdout(self):
        ...
        if instance_id in self.console_widgets and command_index in self.console_widgets[instance_id]:
            console = self.console_widgets[instance_id][command_index]
            try:
    ...
    def on_process_finished(self):
        ...
        if instance_id in self.running_processes and command_index in self.running_processes[instance_id]:
            ...
            # Si ya no quedan procesos para esa instancia, la marcamos como detenida
            if not self.running_processes[instance_id]:
                ...
                self.log_to_console_by_id(instance_id, "--- Todos los procesos de la instancia han finalizado ---")
